...
/**
 * CaféMind - Professional Coffee Kiosk Interface
 * A multi-agent AI-powered coffee ordering system
 * 
 * @author Chidwipak Kuppani
 * @description Main kiosk page featuring product display, search, filtering, and cart management
 */
      ...
      <div className="transform scale-80 origin-top w-full">
          ...
          <div className="max-w-7xl mx-auto px-8 py-8">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center space-x-6">
                ...
                <div>
                  <h1 className="text-4xl font-bold text-premium">CaféMind</h1>
                  <p className="text-xl text-emerald-700 font-semibold">AI Coffee Kiosk</p>
                </div>
              </div>
        ...
        <main className="max-w-7xl mx-auto px-6 py-8">
          ...
          <div className="text-center mb-12">
            ...
            <p className="text-3xl text-emerald-800 font-medium">Choose Your Perfect Coffee Experience</p>
          </div>
          ...
          {products.length > 0 && (
            <div className="mb-12 space-y-8">
              <div className="flex flex-col lg:flex-row gap-8">
                ...
                <div className="relative flex-1 max-w-3xl mx-auto lg:mx-0">
                  <div className="relative">
                    <Search className="absolute left-8 top-1/2 transform -translate-y-1/2 text-emerald-600 h-8 w-8 z-10" />
                    <Input
                      placeholder="Search for your perfect coffee..."
                      value={searchTerm}
                      onChange={(e) => setSearchTerm(e.target.value)}
                      className="pl-20 pr-8 h-20 text-2xl glass-card border-0 rounded-2xl placeholder:text-emerald-600/70 text-emerald-900 font-semibold shadow-lg focus:ring-2 focus:ring-emerald-400"
                ...
                <div className="flex items-center gap-6 flex-wrap justify-center lg:justify-start">
                  <Filter className="h-8 w-8 text-emerald-700" />
                    ...
                    <Button
                      ...
                      className={
                        selectedCategory === category
                          ...
                          : "ceramic-card text-emerald-800 px-8 py-4 text-xl font-bold rounded-2xl hover:text-emerald-900"
                      }
                    >
                      {category}
                    </Button>
              ...
              {/* Results Count */}
              <p className="text-2xl text-emerald-700 font-semibold text-center">
          ...
          {loading ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 px-4">
              {[1, 2, 3, 4, 5, 6, 7, 8, 9].map((i) => (
                <div key={i} className="ceramic-card animate-pulse p-6 rounded-3xl">
                  <div className="bg-gradient-to-br from-emerald-200 to-teal-200 h-56 rounded-2xl mb-6"></div>
                  <div className="bg-gradient-to-r from-emerald-200 to-teal-200 h-8 rounded-2xl mb-6"></div>
                  <div className="bg-gradient-to-r from-emerald-200 to-teal-200 h-6 rounded-xl w-2/3 mb-8"></div>
                  <div className="bg-gradient-to-r from-emerald-200 to-teal-200 h-16 rounded-2xl"></div>
                </div>
              ))}
            </div>
          ) : error ? (
                  ...
                  >
                    <CardContent className="p-0">
                      ...
                      <div className="relative overflow-hidden rounded-2xl mb-6 group">
                        ...
                        />
                        <Badge
                          className="absolute top-6 left-6 glass-card text-emerald-800 font-bold text-lg px-4 py-2 rounded-xl"
                          style={{ zIndex: 20 }}
                        >
                      ...
                      <div className="space-y-6">
                        <div>
                          <h3 className="text-3xl font-bold text-emerald-900 mb-3">{product.name || "Unknown Product"}</h3>
                          <div className="flex items-center space-x-3 mb-4">
                            <Star className="h-6 w-6 fill-emerald-400 text-emerald-400" />
                            <span className="text-xl font-bold text-emerald-800">{product.rating || "N/A"}</span>
                          </div>
                        </div>

                        <p className="text-emerald-700 text-lg line-clamp-3 leading-relaxed font-medium">
                            ...
                            <div className="space-y-6">
                              <div className="flex items-center justify-between glass-card p-6 rounded-2xl">
                                <Button
                                  ...
                                  className="h-16 w-16 p-0 hover:bg-emerald-200/50 rounded-2xl"
                                >
                                  <Minus className="h-8 w-8 text-emerald-800" />
                                </Button>
                                <span className="font-bold text-3xl text-emerald-900">{quantity}</span>
                                <Button
                                  size="lg"
                                  variant="ghost"
                                  onClick={() => handleQuantityChange(product.name, 1)}
                                  className="h-16 w-16 p-0 hover:bg-emerald-200/50 rounded-2xl"
                                >
                                  <Plus className="h-8 w-8 text-emerald-800" />
                                </Button>
                              </div>
                              <div className="text-center glass-card p-4 rounded-2xl">
                                <p className="text-lg text-emerald-700 font-semibold">Subtotal</p>
                                <p className="font-bold text-3xl text-emerald-900">
            ...
            <div className="text-center py-20">
              <div className="w-40 h-40 glass-card rounded-full mx-auto mb-12 flex items-center justify-center float-animation">
                <Coffee className="h-20 w-20 text-emerald-600" />
              </div>
              <h3 className="text-5xl font-bold text-premium mb-8">No Products in Firebase</h3>
              <p className="text-2xl text-emerald-700 mb-12 font-medium">Please add products to your Firebase database.</p>
              <Button
                onClick={handleRetry}
                className="premium-gradient text-white px-12 py-6 text-xl font-bold rounded-2xl shadow-2xl hover:shadow-3xl transition-all duration-300 hover:scale-105"
              >
          ...
          {filteredProducts.length === 0 && !loading && !error && products.length > 0 && (
            <div className="text-center py-20">
              <div className="w-40 h-40 glass-card rounded-full mx-auto mb-12 flex items-center justify-center float-animation">
                <Search className="h-20 w-20 text-emerald-600" />
              </div>
              <h3 className="text-5xl font-bold text-premium mb-8">No items found</h3>
              <p className="text-2xl text-emerald-700 mb-12 font-medium">Try adjusting your search or filter criteria</p>
              <Button
                onClick={() => {
                  setSearchTerm("")
                  setSelectedCategory("All")
                }}
                className="ceramic-card text-emerald-800 px-12 py-6 text-xl font-bold rounded-2xl hover:scale-105 transition-all duration-300"
              >
                Clear Filters
              </Button>
            </div>
